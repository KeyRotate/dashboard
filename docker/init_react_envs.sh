...
if [[ -z "${MILIAN_MGMT_API_ENDPOINT}" ]]; then
    echo "MILIAN_MGMT_API_ENDPOINT environment variable must be set"
    exit 1
fi
...
export MILIAN_MGMT_API_ENDPOINT=$(echo $MILIAN_MGMT_API_ENDPOINT | sed -E 's/(:80|:443)$//')
export MILIAN_MGMT_GRPC_API_ENDPOINT=${MILIAN_MGMT_GRPC_API_ENDPOINT}
export MILIAN_HOTJAR_TRACK_ID=${MILIAN_HOTJAR_TRACK_ID}
export MILIAN_TOKEN_SOURCE=${MILIAN_TOKEN_SOURCE:-accessToken}
export MILIAN_DRAG_QUERY_PARAMS=${MILIAN_DRAG_QUERY_PARAMS:-false}

echo "Milian latest version: ${MILIAN_LATEST_VERSION}"

# replace ENVs in the config
ENV_STR="\$\$USE_AUTH0 \$\$AUTH_AUDIENCE \$\$AUTH_AUTHORITY \$\$AUTH_CLIENT_ID \$\$AUTH_CLIENT_SECRET \$\$AUTH_SUPPORTED_SCOPES \$\$MILIAN_MGMT_API_ENDPOINT \$\$MILIAN_MGMT_GRPC_API_ENDPOINT \$\$MILIAN_HOTJAR_TRACK_ID \$\$AUTH_REDIRECT_URI \$\$AUTH_SILENT_REDIRECT_URI \$\$MILIAN_TOKEN_SOURCE \$\$MILIAN_DRAG_QUERY_PARAMS"
